...
import io.Source
import org.technbolts.mail.{Mailbox, MailboxListener}
...
object Pop3ServerRunner {
  def main(args: Array[String]):Unit = {
    ...
    if(LangUtils.isEmpty(keepStr) || keepStr.equalsIgnoreCase("y")) {
      val ignoreDeleteListener = new MailboxListener {
        override def onCreation(mailbox:Mailbox):Unit = { mailbox.ignoreDelete = true }
      }
      server.mailboxRepository.listeners.append( ignoreDeleteListener )
    }
    server.start
  }
}
